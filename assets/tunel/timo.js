...
function audioPortada(){
  ...
  setVol("forest", .55*estres); setVol("heartbeatFast", .28*estres); setVol("stressLow", .35*estres); setVol("stressHigh", .18*estres); setVol("stressNoise", .25*estres); setVol("rollingBall", .25*estres); setVol("tunnelEcho", .10*estres);
  setVol("heartbeatSoft", .03 + .16*calma); setVol("calmAir", .08 + .35*calma); setVol("fireflies", .06 + .22*calma); setVol("softChimes", .14*calma);
}
...
function cargarEscena(nombre){
  ...
  tunelX = .74; tunelY = .58; pelotaTunelX = .52; pelotaTunelY = .58; pelotaLibre = false;
...
function escenaPortada(){
  ...
  mostrar(pajaro, ASSETS.portada.pajaro, "48%", "28%", "clamp(40px,7vw,85px)");
...
function escenaRespira(){
  ...
  clima({os:.58, ruidoOp:.15, brillo:.68, sat:.72, contraste:1.07, color:"#f0b67a", colorOp:.14});
  crearLuciernagas(7,{x:26,y:56,radio:10});
  mostrar(timo, ASSETS.respira.timoTronco, "24%", "64%", "clamp(120px,25vw,330px)");
  timo.classList.add("temblar");
  mostrar(mariposa, ASSETS.respira.mariposa, "18%", "40%", "clamp(24px,5vw,56px)");
  mostrar(pajaro, ASSETS.respira.pajaro, "12%", "28%", "clamp(34px,6vw,72px)");
  mostrar(luciernagasDibujo, ASSETS.respira.luciernagas, "25%", "52%", "clamp(28px,5vw,70px)");
  mariposa.style.opacity = .25; pajaro.style.opacity = .25;
  spotLuz.style.left = "24%"; spotLuz.style.top = "64%";
  setTexto("Respira", "Mantén presionado a Timo para inhalar. Suelta para exhalar.", "El tronco es su apoyo. Con cada respiración, el mundo se siente más seguro.");
}
...
function moverRespira(paso=1){
  ...
  spotLuz.style.left = `${x}%`; spotLuz.style.top = `${y}%`; spotLuz.style.transform = `translate(-50%,-50%) scale(${1+p*.65})`;
  clima({os:.58-p*.42, ruidoOp:.15-p*.12, brillo:.68+p*.30, sat:.72+p*.25, contraste:1.07-p*.05, color:"#f7c47d", colorOp:.14+p*.18});
  intensidadLuciernagas(.25+p*.45);
...
function escenaTunel(){
  ...
  crearLuciernagas(5,{x:45,y:60,radio:10});
  mostrar(timo, ASSETS.tunel.bolita, "69%", "66%", "clamp(70px,10vw,130px)");
  mostrar(pelota, ASSETS.tunel.pelota, "52%", "66%", "clamp(45px,7vw,90px)");
  pelota.classList.add("flotar");
  spotLuz.style.left = "69%"; spotLuz.style.top = "66%";
  setTexto("El túnel", "Toca para que Timo empuje la pelota hacia la salida", "Timo avanza poco a poco y empuja la pelota por el túnel.");
  actualizarTimoTunel();
}
function distanciaAPelota(){ const dx = pelotaTunelX - tunelX, dy = pelotaTunelY - tunelY; return Math.sqrt(dx*dx + dy*dy); }
function actualizarTimoTunel(){
  const x = 10 + tunelX*80, y = 25 + tunelY*70;
  const px = 10 + pelotaTunelX*80, py = 25 + pelotaTunelY*70;
  timo.style.left = `${x}%`; timo.style.top = `${y}%`; timo.style.transform = `translate(-50%,-50%) rotate(${(1-tunelX)*720}deg)`;
  pelota.style.left = `${px}%`; pelota.style.top = `${py}%`;
  spotLuz.style.left = `${x}%`; spotLuz.style.top = `${y}%`;
  const cerca = 1 - Math.min(distanciaAPelota()/.55,1);
  ...
  intensidadLuciernagas(.18+cerca*.28);
  audioTunel();
}
function avanzarTimoTunelPorTap(){
  if(escenaActual !== "tunel" || pelotaLibre) return;
  const dx = pelotaTunelX - tunelX, dy = pelotaTunelY - tunelY, d = Math.sqrt(dx*dx+dy*dy);
  if(d > .045){ tunelX += dx*.22; tunelY += dy*.22; actualizarTimoTunel(); return; }
  empujarPelotaTunel();
}
function empujarPelotaTunel(){
  playOneShot("ballTap",.62); vibrar(35);
  pelota.classList.remove("saltarPelota"); void pelota.offsetWidth; pelota.classList.add("saltarPelota");
  // Salida definida hacia la izquierda del túnel.
  pelotaTunelX -= .10;
  pelotaTunelY += (Math.random()-.5)*.04;
  tunelX = Math.min(.95, pelotaTunelX + .18);
  tunelY = pelotaTunelY;
  if(pelotaTunelY < .18 || pelotaTunelY > .88){
    playOneShot("bump",.55); vibrar([30,20,40]); setTexto("Ay…", "La pelota rebotó en el túnel", "Timo escucha el eco y vuelve a intentar con cuidado."); pelotaTunelY = Math.max(.18, Math.min(.88, pelotaTunelY));
  } else setTexto("¡Pum!", "La pelota avanza", "Timo la empuja suave hacia la salida.");
  if(pelotaTunelX <= -.04){
    pelotaLibre = true; pelotaTunelX = -.10; pelotaTunelY = .55;
    playOneShot("aplausos",.85); playOneShot("softLaughs",.45); vibrar([40,40,80]);
    setTexto("¡Lo logró!", "Sacaste la pelota del túnel", "Timo encontró su manera de avanzar y salir.");
  }
  actualizarTimoTunel();
}
function golpeTunel(){
...
function escenaFinal(){
  ...
  crearLuciernagas(10,{x:50,y:50,radio:28});
  mostrar(luciernagasDibujo, ASSETS.final.luciernagas, "50%", "50%", "clamp(70px,14vw,160px)");
  mostrar(timo, ASSETS.final.timo, "30%", "76%", "clamp(95px,14vw,170px)");
  mostrar(mono, ASSETS.final.mono, "70%", "58%", "clamp(95px,15vw,180px)"); mono.classList.add("flotar");
  mostrar(conejo, ASSETS.final.conejo, "17%", "80%", "clamp(70px,11vw,140px)");
  mostrar(pajaro, ASSETS.final.pajaro, "55%", "24%", "clamp(45px,7vw,85px)"); pajaro.classList.add("flotar");
  mostrar(mariposa, ASSETS.final.mariposa, "45%", "42%", "clamp(30px,5vw,60px)"); mariposa.classList.add("flotar");
  mostrar(pelota, ASSETS.final.pelota, "58%", "78%", "clamp(45px,8vw,95px)");
  turnoPelotaFinal = 0; pelota.style.transition = "left .55s ease-out, top .55s ease-out, transform .55s ease-out"; pelota.classList.add("pelotaInteractiva");
  spotLuz.style.left = "50%"; spotLuz.style.top = "60%"; spotLuz.style.transform = "translate(-50%,-50%) scale(2.2)";
  intensidadLuciernagas(.42);
  setTexto("Juguemos todos", "Toca la pelota para lanzarla", "La pelota vuelve para seguir jugando con todos.");
  audioFinal();
}
const destinosPelotaFinal = [
  {nombre:"Timo", x:"30%", y:"76%"}, {nombre:"Conejo", x:"17%", y:"80%"}, {nombre:"Pájaro", x:"55%", y:"24%"}, {nombre:"Mariposa", x:"45%", y:"42%"}, {nombre:"Mono", x:"70%", y:"58%"}
];
function lanzarPelota(){
  ...
  pelota.style.left=destino.x; pelota.style.top=destino.y; pelota.style.transform="translate(-50%,-50%) scale(1.15) rotate(180deg)";
  playOneShot("playBall",.75); playOneShot("softLaughs",.35); setVol("happyWind",.55); vibrar(35);
  setTexto("¡Va para " + destino.nombre + "!", "Toca otra vez la pelota", "La pelota vuelve para seguir jugando con todos.");
  setTimeout(()=>{ pelota.style.left=origen.x; pelota.style.top=origen.y; pelota.style.transform="translate(-50%,-50%) scale(1) rotate(0deg)"; }, 650);
}
...
window.addEventListener("pointerdown", e => { pointerActivo=true; ultimaY=e.clientY; if(escenaActual === "respira" && e.target === timo) iniciarRespira(); });
